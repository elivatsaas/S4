...
export default function AddShiftPage() {
  ...
  const handleSubmit = async () => {
    ...
    //   selectedFilters.roleIds,
    //   selectedFilters.storeIds,
    //   selectedFilters.scheduleIds
    // );
    ...
    if (
      ...
      if (date.length === 0) {
        ...
        const shiftAdded = await addShift({
          date,
          startTime,
          endTime,
          Employee_id,
        ...
        if (shiftAdded.status === "success") {
          alert("Submitted!");
          window.location.href = "/addshiftpage";
        } else {
          alert("Failed to submit!");
        }
      }
    } else {
      if (selectedFilters.employeeIds.length > 1) {
        alert("Multiple Employees Selected!");
      } else if (
        selectedFilters.roleIds.length === 0 ||

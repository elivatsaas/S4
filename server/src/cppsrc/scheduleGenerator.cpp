...
extern "C"
{
    ...
    int main(int argc, const char *argv[])
    {
        ...
        // ifstream shiftFile("inputData/test2Shifts.json",ifstream::binary);
        // shiftFile >> shiftJson;
        // shiftCount = 8;

        // ifstream employeeForScheduleFile("inputData/test2EmployeesForSchedule.json",ifstream::binary);
        // employeeForScheduleFile >> employeeShiftJson;
        // employeeShiftCount = 14;

        // ifstream employeeFile("inputData/test2Employees.json",ifstream::binary);
        // employeeFile >> employeeJson;
        // employeeCount = 3;

        // ifstream hoursFile("inputData/test2Hours.json",ifstream::binary);
        // hoursFile >> hoursJson;
        ...
        for (int i = 0; i < shiftCount; i++)
        {
            ...
            startTime = shiftJson[i]["startTime"].asString();
            endTime = shiftJson[i]["endTime"].asString();

            shifts.sftArr[i].startTime = convertTime(startTime);
            shifts.sftArr[i].endTime = convertTime(endTime);
            shifts.sftArr[i].id = shiftJson[i]["id"].asInt();

            // no preset employee
            if (shiftJson[i]["Employee_id"].asString() == "\0")
            {
                // cout << "Null worked!" << endl;
                shifts.sftArr[i].empId = -1;
            }
            // preset employee
            else
            {
                ...
                int validEmployee = employeeExists(&employees, shifts.sftArr[i].empId);
                // check to make sure that is a valid employee
                if (validEmployee >= 0)
                {
                    addShiftToEmployee(shifts.sftArr[i], validEmployee, &employees);
            ...
            if (i > 0 && !sameWeek(shifts.sftArr[i - 1], shifts.sftArr[i]))
            {
        ...
        else
        {
            ...
            cout << incomplete.sftArr << endl;
            incomplete.shiftCount = shifts.shiftCount;
            //  uncomment when ready for online connection eli
